...
def test_adapt_query_geography_artifacts_falls_back_to_flickr_query_definitions_on_non_string_path(
    tmp_path: Path,
) -> None:
    ...
    source_manifest["outputs"]["flickr_query_definitions"] = "query_definitions.json"

    manifest_path = tmp_path / "run_manifest_query_geography_fallback_query_definitions.json"
...
def test_adapt_query_geography_artifacts_falls_back_to_geographic_spread_on_non_string_path(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_query_geography.json").read_text(
    ...
    source_manifest["outputs"]["taxon_geographic_spread"] = {}
    source_manifest["outputs"]["geographic_spread"] = "taxon_geographic_spread.json"

    manifest_path = tmp_path / "run_manifest_query_geography_fallback_spread.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    ...
    notes = result["compatibility"]["notes"]
    assert not any(
        "did not declare taxon_geographic_spread artifact path" in note for note in notes
    )
    assert not any("failed to parse taxon_geographic_spread artifact" in note for note in notes)
...
def test_adapt_query_geography_artifacts_non_list_rows_as_null_blocks_query_definitions(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_rows_null_with_query_definitions.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_rows_null_with_query_definitions.json").write_text(
        json.dumps(
            {
                "rows": None,
                "query_definitions": [
                    {
                        "query_definition_id": "qd-query-definition-ignored",
                        "query_eligible": True,
                        "enabled": True,
                    }
                ],
    ...
    # Keep remaining artifacts valid so this test isolates the rows-key fallback rule.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_treats_scalar_rows_as_no_rows_over_query_definitions(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    # Keep remaining artifacts valid so this test isolates scalar rows-key authority.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_prefers_empty_rows_key_over_query_definitions_key(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_rows_empty_with_query_definitions.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_rows_empty_with_query_definitions.json").write_text(
        json.dumps(
            {
                "rows": [],
                "query_definitions": [
    ...
    # Keep remaining artifacts valid so this test isolates row-priority behavior.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_non_list_rows_key_blocks_data_key(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_rows_bad_with_data_key.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_rows_bad_with_data_key.json").write_text(
        json.dumps(
            {
                "rows": {"query_definition_id": "qd-bad-rows"},
                "data": [
                    {
                        "query_definition_id": "qd-data-ignored",
                        "query_eligible": True,
                        "enabled": True,
                    }
                ],
    ...
    # Keep remaining artifacts valid so this test isolates query-definition key precedence.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_treats_scalar_data_as_no_rows_over_query_definitions(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = (
        tmp_path / "run_manifest_query_geography_scalar_data_with_query_definitions.json"
    )
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_scalar_data_with_query_definitions.json").write_text(
        json.dumps(
            {
                "data": "bogus",
                "query_definitions": [
    ...
    # Keep remaining artifacts valid so this test isolates data-key authority.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_non_list_rows_blocks_candidates_key(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_rows_non_list_with_candidates.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_rows_non_list_with_candidates.json").write_text(
        json.dumps(
            {
                "rows": {
                    "query_definition_id": "qd-rows-not-list",
                    "query_eligible": True,
                    "enabled": True,
                },
                "candidates": [
    ...
    # Keep remaining artifacts valid so this test isolates rows-key authority.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
...
def test_adapt_query_geography_artifacts_treats_non_list_query_definitions_key_as_no_rows(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_query_definitions_non_list.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_query_definitions_non_list.json").write_text(
        json.dumps(
            {
                "query_definitions": {
                    "query_definition_id": "qd-query-definitions-not-list",
                    "query_eligible": True,
                    "enabled": True,
                },
                "candidates": [
...
def test_adapt_query_geography_artifacts_prefers_rows_key_over_query_definitions_key(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_rows_over_query_definitions.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_rows_over_query_definitions.json").write_text(
        json.dumps(
            {
                "rows": [
                    {
                ...
                "query_definitions": {
                    "query_definition_id": "qd-query-definitions-not-list-ignored",
                    "query_eligible": False,
                    "enabled": False,
                },
            }
    ...
    # Keep other artifacts valid so this test isolates extraction-order behavior.
    (tmp_path / "taxon_geographic_spread.json").write_text(
        Path("packages/replay/tests/fixtures/taxon_geographic_spread.json").read_text(
            encoding="utf-8"
        ),
    ...
    assert len(result["query_definitions"]) == 1
    assert result["query_definitions"][0]["query_definition_id"] == "qd-rows-001"
    assert result["query_definitions"][0]["search_priority"] == 7
    assert result["query_definition_summary"]["total_query_definitions"] == 1
    assert result["query_definition_summary"]["eligible_query_definitions"] == 1
    assert result["query_definition_summary"]["query_curation_rule_count"] == 0
...
def test_adapt_query_geography_artifacts_ignores_null_query_definitions_payload(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_null.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "query_definitions_null.json").write_text("null", encoding="utf-8")
...
def test_adapt_query_geography_artifacts_treats_null_spread_payload_as_no_rows(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_null_spread.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "taxon_geographic_spread_null.json").write_text("null", encoding="utf-8")

    # Keep other artifacts valid so this test isolates spread payload behavior.
    (tmp_path / "query_definitions.json").write_text(
        Path("packages/replay/tests/fixtures/query_definitions.json").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_adapt_query_geography_artifacts_treats_null_occurrence_payload_as_no_rows(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
...
def test_adapt_query_geography_artifacts_treats_null_summary_payload_as_no_rows(
    tmp_path: Path,
) -> None:
    source_manifest = json.loads(
        Path(
    ...
    manifest_path = tmp_path / "run_manifest_query_geography_null_summary.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
    (tmp_path / "taxon_geographic_summary_null.json").write_text("null", encoding="utf-8")

    # Keep other artifacts valid so this test isolates summary payload behavior.
    (tmp_path / "query_definitions.json").write_text(
        Path("packages/replay/tests/fixtures/query_definitions.json").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_adapt_query_geography_artifacts_treats_all_output_values_as_blank_as_missing(tmp_path: Path) -> None:
    ...
    source_manifest["outputs"]["flickr_query_definitions"] = " "
    source_manifest["outputs"]["taxon_geographic_spread"] = "  "
    source_manifest["outputs"]["geographic_spread"] = "\t"
    source_manifest["outputs"]["geographic_occurrence_evidence"] = "  "
    source_manifest["outputs"]["occurrence_evidence"] = ""
    source_manifest["outputs"]["geographic_spread_manifest"] = " \n "
    source_manifest["outputs"]["taxon_geographic_summary"] = " "
    source_manifest["outputs"]["geographic_summary"] = ""
    source_manifest["outputs"]["geographic_summary_manifest"] = " "
    source_manifest["outputs"]["geographic_summary_manifest_file"] = ""
    source_manifest["outputs"]["geographic_qa_findings"] = " "
    source_manifest["outputs"]["taxon_geographic_qa_findings"] = " "

    manifest_path = tmp_path / "run_manifest_query_geography_all_outputs_blank.json"
    manifest_path.write_text(json.dumps(source_manifest), encoding="utf-8")
