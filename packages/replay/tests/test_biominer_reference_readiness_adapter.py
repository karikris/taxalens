...
def test_adapt_reference_readiness_permits_vision_stringized(tmp_path: Path) -> None:
    ...
    readiness_payload["permits_vision"] = "yes"
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = tmp_path / "run_manifest_reference_readiness_permits_vision_yes.json"
...
def test_adapt_reference_readiness_permits_vision_unknown_value_is_none(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    summary = result["reference_readiness_summary"]
    assert summary is not None
    assert summary["permits_vision"] is None
...
def test_adapt_reference_readiness_skips_non_mapping_check_in_middle(tmp_path: Path) -> None:
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_middle_bad_check.json"
    artifact_path = tmp_path / "reference_bank_readiness.json"
    manifest_payload["outputs"]["reference_readiness_manifest"] = str(artifact_path.name)
    ...
    assert summary is not None
    assert len(checks) == 2
    assert checks[0]["check_id"] == expected_first_check_id
    assert checks[1]["check_id"] == expected_last_check_id
    assert summary["checks_total"] == 2
    assert summary["check_ids"][0] == expected_first_check_id
    assert summary["check_ids"][1] == "readiness_check_1"
    assert summary["check_ids"][2] == expected_last_check_id
    assert result["compatibility"]["checks_read"] == 2
    assert "non_mapping_readiness_check_1" in result["compatibility"]["skipped_checks"]
    assert "skipped 1 readiness checks while adapting" in result["compatibility"]["notes"]


def test_adapt_reference_readiness_skips_multiple_non_mapping_checks_with_indexed_ids(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    original_checks = readiness_payload["checks"]
    expected_first_check_id = original_checks[0].get("check_id", "readiness_check_0")
    expected_second_check_id = original_checks[1].get("check_id", "readiness_check_1")
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_multiple_bad_checks.json"
    artifact_path = tmp_path / "reference_bank_readiness.json"
    manifest_payload["outputs"]["reference_readiness_manifest"] = str(artifact_path.name)
    ...
    assert len(checks) == 2
    assert checks[0]["check_id"] == expected_first_check_id
    assert checks[1]["check_id"] == expected_second_check_id
    assert summary["checks_total"] == 2
    assert summary["check_ids"][0] == expected_first_check_id
    assert summary["check_ids"][1] == "readiness_check_1"
    assert summary["check_ids"][2] == "readiness_check_2"
    assert summary["check_ids"][3] == expected_second_check_id
    assert summary["check_ids"][4] == "readiness_check_4"
    assert result["compatibility"]["checks_read"] == 2
    assert result["compatibility"]["skipped_checks"] == [
        "non_mapping_readiness_check_1",
        "non_mapping_readiness_check_2",
        "non_mapping_readiness_check_4",
    ]
    assert "skipped 3 readiness checks while adapting" in result["compatibility"]["notes"]
...
def test_adapt_reference_readiness_rejects_boolean_schema_version(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_vtrue.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_rejects_false_schema_version(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    )
    manifest_payload["schema_version"] = False

    manifest_path = tmp_path / "run_manifest_reference_readiness_vfalse.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_rejects_whitespace_schema_version(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_vspace.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_rejects_float_schema_version(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_vfloat.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_rejects_whitespace_only_schema_version(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_vspace_only.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_missing_run_id_falls_back_to_unknown(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_missing_run_id.json"
    artifact_path = tmp_path / "reference_bank_readiness.json"
    manifest_payload["outputs"]["reference_readiness_manifest"] = artifact_path.name
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    artifact_path.write_text(json.dumps(readiness_payload), encoding="utf-8")
    ...
    assert result["reference_readiness_summary"] is not None
    assert result["reference_readiness_summary"]["run_id"] == "unknown-run"


def test_adapt_reference_readiness_blank_run_id_falls_back_to_unknown(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_blank_run_id.json"
    artifact_path = tmp_path / "reference_bank_readiness.json"
    manifest_payload["outputs"]["reference_readiness_manifest"] = artifact_path.name
...
def test_adapt_reference_readiness_trims_reference_readiness_manifest_path(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
...
def test_adapt_reference_readiness_trims_reference_readiness_directory_output_path(
    ...
    assert result["reference_readiness_summary"]["readiness_artifact_path"] == str(
        artifact_directory / "reference_bank_readiness.json"
    )


def test_adapt_reference_readiness_reference_manifest_output_blank_is_missing(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    manifest_payload["outputs"]["reference_readiness_manifest"] = "   "
    manifest_payload["outputs"]["reference_readiness"] = None
    manifest_payload["outputs"]["readiness_manifest"] = 0
    manifest_payload["outputs"]["reference_readiness_file"] = ""
    manifest_payload["outputs"]["readiness"] = {}

    manifest_path = tmp_path / "run_manifest_reference_readiness_blank_output_value.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
...
def test_adapt_reference_readiness_blank_manifest_key_falls_back(
    ...
    manifest_payload["outputs"] = {
        "reference_readiness_manifest": "   ",
        "reference_readiness": "reference_readiness_payload.json",
    }
    manifest_path = tmp_path / "run_manifest_reference_readiness_output_key_fallback.json"
    artifact_path = tmp_path / "reference_readiness_payload.json"
...
def test_adapt_reference_readiness_none_candidate_set_ids_treated_as_empty(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    manifest_path = tmp_path / "run_manifest_reference_readiness_none_candidate_set_ids.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
    )
...
def test_adapt_reference_readiness_non_list_candidate_set_ids_treated_as_empty(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    readiness_payload["candidate_set_ids"] = {"a": 1}
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = tmp_path / "run_manifest_reference_readiness_non_list_candidate_set_ids.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
    )
...
def test_adapt_reference_readiness_non_list_candidate_set_fingerprints_treated_as_empty(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    readiness_payload["candidate_set_fingerprints"] = {"k": "v"}
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = (
        tmp_path / "run_manifest_reference_readiness_non_list_candidate_set_fingerprints.json"
    )
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
...
def test_adapt_reference_readiness_non_list_affected_collections_count_as_zero(
    ...
    readiness_payload["checks"][0]["affected_species"] = None
    readiness_payload["checks"][0]["affected_clusters"] = {}
    ...
    checks = result["reference_readiness_checks"]
    assert checks[0]["affected_species_count"] == 0
    assert checks[0]["affected_clusters_count"] == 0
    assert checks[0]["affected_routes_count"] == 0
...
def test_adapt_reference_readiness_nonnumeric_string_counts_in_checks_are_ignored(
    tmp_path: Path,
) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
    ...
    readiness_payload["checks"][0]["observed"] = "not-a-number"
    readiness_payload["checks"][0]["required"] = "also-bad"
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = tmp_path / "run_manifest_reference_readiness_nonnumeric_check_counts.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
    )
    ...
    assert check["observed_count"] is None
    assert check["required_count"] is None
    assert check["observed_type"] == "string"
    assert check["required_type"] == "string"


def test_adapt_reference_readiness_float_counts_in_checks_are_ignored(tmp_path: Path) -> None:
    manifest_payload = json.loads(
        Path("packages/replay/tests/fixtures/run_manifest_reference_readiness.json").read_text(
            encoding="utf-8"
        )
    ...
    readiness_payload["checks"][0]["observed"] = 2.5
    readiness_payload["checks"][0]["required"] = 1.25
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = tmp_path / "run_manifest_reference_readiness_float_check_counts.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
    )
    ...
    assert check["observed_count"] is None
    assert check["required_count"] is None
    assert check["observed_type"] == "float"
...
def test_adapt_reference_readiness_boolean_counts_are_not_treated_as_integers(
    ...
    readiness_payload["counts"] = {
        "support_manifest_rows": True,
        "eligible_support_rows": False,
    }
    manifest_payload["outputs"]["reference_readiness_manifest"] = "reference_bank_readiness.json"

    manifest_path = tmp_path / "run_manifest_reference_readiness_boolean_counts.json"
    manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
    (tmp_path / "reference_bank_readiness.json").write_text(
        json.dumps(readiness_payload), encoding="utf-8"
    )
