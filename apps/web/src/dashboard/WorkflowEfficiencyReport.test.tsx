...
describe('WorkflowEfficiencyReport', () => {
  ...
  it('renders measured evidence state and complete table provenance', () => {
    ...
    const completeness = within(metrics)
      ...
      .closest('article')
    expect(completeness).toHaveTextContent('39 of 39 artifacts verified')
    expect(completeness).toHaveTextContent('Available sections14')
    expect(completeness).toHaveTextContent('Partial sections11')
    ...
    expect(screen.getByText('Integrity is not scientific completeness').parentElement?.parentElement)
      .toHaveTextContent('14 are available, 11 partial, and 6 unavailable')
