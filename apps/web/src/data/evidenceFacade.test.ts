...
describe('loadEvidenceFacade', () => {
  it('validates the committed contract and verifies every artifact in deterministic order', async () => {
    ...
    expect(facade.replay.artifactCount).toBe(28)
    expect(facade.replay.verifiedArtifactCount).toBe(28)
    expect(facade.replay.unavailableSections).toHaveLength(10)
    expect(facade.replay.sections.yoloe_evidence.status).toBe('unavailable')
    expect(facade.replay.artifactInventory).toHaveLength(28)
    ...
    expect(facade.replay.geographyReference).toEqual({
      ...
      sourceRights: {
        ...
        licensedImageCount: 0,
        mediaPolicy:
          'Verification media is admitted only with a committed licence, attribution, content checksum, byte count, review item, and non-scientific use scope.',
      },
    })
